var Tests = {
	makeGL: function() {
		...
		this.gl = new GL("graph-container", [0.5, 1, 1], 0.1, 3, cam_args);
	},

	makeAnimation: function() {
		this.animation = new Animation(1, 1, gl);
	}
	...
	testGraph: function() {
		...
		var graph = new Graph(gl, [0.5, 0.5, 1], false, 0.3, 10, 10);
		var animation = new Animation(1, 1, gl);
		animation.startRender();

		var l = new Line();
		var v1 = new THREE.Vector4(0, 0, 0, 0);
		var v2 = new THREE.Vector4(1, 1, 1, 1);
		l.aliasV1(v1);
		l.aliasV2(v2);

		var p = new THREE.Vector4(-1, -1, -1, -1);

		graph.plotTube(l);
		graph.plotSphere(p);


		graph.updateMaterials([1, 1, 0.5], true);
		graph.updateTubeShape(0.6, 20);
	}

	testMeshCollapse: function() {
		var circle = new THREE.CircleGeometry(0.5, 10);
		var shape = new THREE.Shape(circle.vertices.slice(1, 11));

		var test = new THREE.ExtrudeGeometry(shape, {extrudePath: new THREE.LineCurve3(
			new THREE.Vector4(1, 1, 1, 1),
			new THREE.Vector4(1, 1, 1, 1)
		)});

		var mesh = new THREE.Mesh(test, new THREE.MeshLambertMaterial());

		mesh.geometry = new THREE.ExtrudeGeometry(shape, {extrudePath: new THREE.LineCurve3(
